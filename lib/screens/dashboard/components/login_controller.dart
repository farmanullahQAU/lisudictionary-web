...
try {
 final UserCredential userCredential= await Get.find<AuthService>().signUpWithEmailAndPassword(
      email: emailController?.text, password: passwordController?.text);
  //After signup add user details to firestore
  final user = UserModel(email: emailController?.text,name: nameController?.text,isAdmin: false);
  await Get.find<FirestoreService>().addUser(user,userCredential.user?.uid);
  Get.back();
}
catch(error){
  print("error");
  ...
  async {
    try {
  final UserCredential user=    await Get.find<AuthService>().loginWithEmailAndPassword(
          email: emailController?.text, password: passwordController?.text);

...
Future<List<Word>> getWords()async{
    ...
    return await _firestore.collection("words").get().then((value) => value.docs.map((e) => Word.fromJson(e.data(), e.id)).toList());
}

Future check(String id,bool action)
async {

   return await _firestore.collection("words").doc(id).update({"isVerified":action});
}
}